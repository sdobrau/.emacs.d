...
In addition to any hooks its parent mode `emacs-lisp-mode' might have run, this
mode runs the hook `pdf-virtual-edit-mode-hook', as the final or penultimate
...
In addition to any hooks its parent mode `pdf-view-mode' might have run, this
mode runs the hook `pdf-virtual-view-mode-hook', as the final or penultimate
step during initialization.
