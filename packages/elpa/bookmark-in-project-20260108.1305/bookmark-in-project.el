...
;; Package-Version: 20260108.1305
;; Package-Revision: bf923ddd3e74
...
where the first argument is always ELEMENT, the second is a member of XS."
  ...
  (let ((xs-length (length xs))
        (index 0)
        (test-fn (or (plist-get args :test) #'equal)))
    (while (and (< index xs-length) (null (funcall test-fn element (aref xs index))))
      (incf index))
...
(defun bookmark-in-project--repr-percent (value-fraction value-total)
...
(defmacro bookmark-in-project--with-save-deferred (&rest body)
  "Execute BODY with bookmark-saving deferred."
    ...
    (cond
        ...
        (let ((lines-rel (count-lines pos-best pos)))
          ...
          (when (< 1 lines-rel)
            (let ((lines-all (count-lines pos-best pos-next)))
              (setq text
                    (concat
                     text " [" (bookmark-in-project--repr-percent lines-rel lines-all) "]")))))
     ...
     (t ; No context, show the percent in the file.
      ...
      (let ((lines-rel (count-lines (point-min) pos))
        ...
        (concat "[" (bookmark-in-project--repr-percent lines-rel lines-all) "]"))))))

(defun bookmark-in-project-name-default-with-line ()
  "Return the name used to create the bookmark with the line number."
  (declare (important-return-value t))
...
(defun bookmark-in-project-name-default-with-context ()
  "Return the name used to create the bookmark with contextual information."
  (declare (important-return-value t))
  (let ((filepath (bookmark-in-project--canonicalize-path (buffer-file-name))))
          ...
          (when (buffer-narrowed-p)
            (save-excursion
              (let ((point-narrow-min (point-min))
                ...
                (save-restriction
                  (widen)
                  ;; Instead of calling `bookmark-jump', call this lower level function
                  ;; so as not to create messages or set the `bookmark-current-bookmark'.
...
Note that this must only run for bookmarks in the current buffer,
  ...
  (let* ((filepath-item (bookmark-in-project--item-get-filename (car pi)))
    ...
    (while (< i filepath-bm-list-pairs-len)
      (pcase-let ((`(,filepath-iter . ,pi-list-iter) (aref filepath-pi-list-pairs i)))
        (cond
         ;; The file has been met, find the index within the list.
...
(defun bookmark-in-project--remap-name (bm-list-src bm-list-dst name)
  "Translate NAME from its value in BM-LIST-SRC to its value in BM-LIST-DST."
  (declare (important-return-value t))
  (let ((result name))
    (while bm-list-src
